...
class MOBODYEnsembleDynamics(BaseDynamics):
    ...
    def encode_action_effect(self, state, action=None, next_state=None, trg=True, mode="delta_state", reduce_ensemble=True):
        """Wrapper used by policy-stage reliability calibration.

        It intentionally does not alter dynamics training; it only exposes the
        learned MOBODY latent space for downstream rollout/BC calibration.
        """
        self.model.inference()
        state = self.obs_scaler.transform_tensor(state)
        if next_state is not None:
            next_state = self.obs_scaler.transform_tensor(next_state)
        return self.model.encode_action_effect(
            state=state,
            action=action,
