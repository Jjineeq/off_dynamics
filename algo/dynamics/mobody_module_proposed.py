...
class MOBODYModule(nn.Module):
    ...
    @torch.no_grad()
    def encode_action_effect(self, state, action=None, next_state=None, trg=True, mode="delta_state", reduce_ensemble=True):
        """Encode a transition-level action-effect latent.

        mode="delta_state": z = E(s') - E(s). This directly uses the realized
        state change and is the default reliability signal.
        mode="encoded_action": z = A_trg(E(s), a) or A_src(E(s), a). This uses
        the domain-specific action encoder.
        """
        was_training = self.training
        self.inference()

        zs, _, _ = self.encode_state(state)

        if mode == "encoded_action":
            if action is None:
                raise ValueError("action is required when mode='encoded_action'")
            if trg:
                z = self.encode_trg_action(zs, action, False)
            else:
                z = self.encode_src_action(zs, action, False)
        elif mode == "delta_state":
            if next_state is None:
                raise ValueError("next_state is required when mode='delta_state'")
            zs_next, _, _ = self.encode_state(next_state)
            z = zs_next - zs
        else:
            raise ValueError(f"Unknown action-effect mode: {mode}")

        if reduce_ensemble:
            if hasattr(self, "elites"):
                elite_idx = self.elites.data.long().to(z.device)
                z = z.index_select(0, elite_idx).mean(dim=0)
            else:
                z = z.mean(dim=0)
