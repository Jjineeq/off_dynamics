...
try:
    from algo.mobody_proposed_v2 import MOBODY as ProposedMOBODY
except ImportError:
    ProposedMOBODY = None
...
from algo.dynamics.mobody_dynamics_proposed import MOBODYEnsembleDynamics
from algo.dynamics.mobody_module_proposed import MOBODYModule
import wandb
...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy", default="mobody", help='policy to use')
    ...
    parser.add_argument('--shift_level', default=1, help='the scale of the dynamics shift. Note that this value varies on different settins')
    ...
    # proposed: latent reliability calibration
    parser.add_argument('--use_latent_reliability', default=0, type=int,
                        help='Enable latent action-effect reliability calibration.')
    parser.add_argument('--reliability_target', default='bc', type=str,
                        choices=['none', 'bc', 'rollout', 'both'],
                        help='Where to apply latent reliability weights.')
    parser.add_argument('--reliability_tau', default=1.0, type=float,
                        help='Temperature for exp(-distance/tau).')
    parser.add_argument('--reliability_min', default=0.1, type=float,
                        help='Minimum reliability clamp.')
    parser.add_argument('--reliability_max', default=1.0, type=float,
                        help='Maximum reliability clamp.')
    parser.add_argument('--reliability_metric', default='l2', type=str,
                        choices=['l2', 'cosine'],
                        help='Distance metric in action-effect latent space.')
    parser.add_argument('--reliability_effect_mode', default='delta_state', type=str,
                        choices=['delta_state', 'encoded_action'],
                        help='Latent action-effect definition.')
    parser.add_argument('--reliability_proto_max_samples', default=50000, type=int,
                        help='Max observed target transitions used to estimate the target latent prototype.')
    parser.add_argument('--reliability_proto_chunk_size', default=4096, type=int,
                        help='Chunk size for target prototype computation.')
    parser.add_argument('--reliability_proto_update_interval', default=0, type=int,
                        help='0 means fixed prototype after first computation; positive value updates every N policy steps.')
    parser.add_argument('--reliability_normalize_mean', default=0, type=int,
                        help='Normalize reliability to have mean one before clamping.')
    parser.add_argument('--reliability_debug_interval', default=1000, type=int,
                        help='Print reliability debug info every N policy steps. Use 0 to disable periodic debug.')
...
# use_src_sa_to_get_target_next_state
    ...
    config.update({
        ...
        'use_latent_reliability': args.use_latent_reliability,
        'reliability_target': args.reliability_target,
        'reliability_tau': args.reliability_tau,
        'reliability_min': args.reliability_min,
        'reliability_max': args.reliability_max,
        'reliability_metric': args.reliability_metric,
        'reliability_effect_mode': args.reliability_effect_mode,
        'reliability_proto_max_samples': args.reliability_proto_max_samples,
        'reliability_proto_chunk_size': args.reliability_proto_chunk_size,
        'reliability_proto_update_interval': args.reliability_proto_update_interval,
        'reliability_normalize_mean': args.reliability_normalize_mean,
        'reliability_debug_interval': args.reliability_debug_interval,
    ...
    if args.policy.lower() == 'mobody' and ProposedMOBODY is not None:
        # Use the proposed MOBODY implementation when algo/mobody_proposed.py exists.
        policy = ProposedMOBODY(config, device)
    else:
